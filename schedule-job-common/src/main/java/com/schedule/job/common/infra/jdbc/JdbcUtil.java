...
public class JdbcUtil {
    ...
    /**
     * 如果存在自增的主键ID，在插入后返回主键id
     */
    public static Long executeInsertAndGetId(String sql, Object... params) throws SQLException {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet generatedKeys = null;
        try {
            conn = JdbcClient.getConnection();
            // 使用 RETURN_GENERATED_KEYS 标志，让 PreparedStatement 返回自增ID
            ...
            if (rows > 0) {
                // 插入成功后，调用getGeneratedKeys()方法返回主键Id
                generatedKeys = pstmt.getGeneratedKeys();
                if (generatedKeys.next()) {
                    Long generatedId = generatedKeys.getLong(1);
                    log.debug("插入成功，获取自增ID：{}", generatedId);
                    return generatedId;
                }
            }
            log.warn("插入成功但未获取到自增ID，SQL：{}", sql);
            return null;
        } catch (SQLException e) {
            log.error("数据库插入失败", e);
            throw new RuntimeException("执行插入SQL失败", e);
        } finally {
